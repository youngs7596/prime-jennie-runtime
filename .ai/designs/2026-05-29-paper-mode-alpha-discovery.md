...
### 1.1 실계좌 연결 현황 — 정체성과 인프라의 어긋남을 명시 (2026-06-03)

위 정체성은 "실계좌에서 손을 뗀다" 이지만, **인프라는 실계좌에 연결된 채로 남아 있다.**
민지 검토 (2026-05-29 의뢰서 §3-2) 는 계좌 분리 (KIS paper 계좌 전환) 를 권고했으나
영석님이 보류했다 — STOP 유지로 매매 차단은 충분하고, 필요할 때 텔레그램으로 바로
제어할 수 있는 편의가 더 크다는 판단. 분리 대신 이 절로 현황을 기록한다.

현재 상태 (2026-06-03 기준):

- KIS gateway 는 실계좌 (68211289) 에 연결되어 있다. 매매는 control STOP 이 전면 차단
- 실계좌 보유 6종목은 v3 가 건드리지 않는 동결 상태 — KODEX200 1,568주 (주력),
  두산밥캣 41주, KB금융 68주, 나머지 셋은 1주씩
- 긴급 청산 경로: 운영자가 KIS 앱에서 직접, 또는 텔레그램 /resume 후 청산 명령.
  STOP 상태에서는 텔레그램 청산 명령도 막힌다 (모든 매매 차단이 STOP 의 정의)
- 이 구성의 의미: "v3 가 실계좌를 매매하지 않는다" 는 보장은 코드 구조가 아니라
  **STOP 이라는 운영 상태**가 제공한다. STOP 을 푸는 순간 그 보장이 사라진다는 것을
  인지한 상태에서만 풀어야 하며, paper 증명이 통과하기 전에는 풀 이유가 없다

> **2026-06-12 수정**: 위 절의 "실계좌에서 손을 뗀다" 에 예외 하나가 정식화됐다 —
> 운영자가 텔레그램으로 직접 지시하거나 추천을 수락한 매매는 실계좌로 나간다
> (자율 매매는 계속 없음). 유사시 처분이라는 임시 통로를 정식 기능으로 올린 것.
> 목표 운영 상태도 STOP 에서 PAUSE (진입 차단·청산 허용) 로 바뀐다.
> 상세: `.ai/designs/2026-06-12-human-approved-trading-nl-interface.md`
...
> **정정 (5-29, P2.5)**: 위 "paper outcomes" 가 측정을 돌리고 있다는 전제는 도입
> 시점에 사실이 아니었다. 5-27 도입 이래 진입가 결손 시트(`entry_price=None`)가
> 022 의 NOT NULL 제약에 막혀 매일 배치 전체가 롤백됐고, 측정 결과는 0 건이었다.
> 운영 실측으로 드러났다 (검토 의뢰서 `.ai/reviews/2026-05-29-paper-mode-review-request.md`
> §2-2). P2.5 에서 migration 023(제약 해제) + per-row 격리 + data_missing 기록 +
> 측정 잡 실패 alert 로 복구했다. §5 의 "측정 → 재측정" 루프는 이 복구 이후부터
> 실제로 돈다.
