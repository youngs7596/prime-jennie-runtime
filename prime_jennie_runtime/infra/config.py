...
class KISConfig(BaseSettings):
    ...
    # ~18-20/sec 에서 정상 동작.
    #
    # 2026-06-05 스택 정지 후 토큰버킷 조합을 직접 스윕해 두 가지를 확정:
    #   - 거부(EGW00201)는 burst 와 무관하다. 순간 동시 20건까지 통과(25건부터 거부),
    #     같은 평균이면 몰아 쏘는 쪽이 균등보다 거부가 적다 → KIS 는 1초 누적량을 본다.
    #   - 거부율은 지속 평균 rate 에만 달렸다: rate 5=0%, 9=2%, 10=6~8%, 15=9%, 18=12%.
    # rate 10 의 6~8% 거부를 없애려 9 로 낮춘다 (거부 ~2%, 실효 9.3→9.0/sec 로 거의 동일).
    rate_limit_global_per_sec: int = 9
    # burst 는 거부와 무관함이 위 실측에서 확인됨 — 줄여도 거부가 안 줄어 4 로 유지한다.
    rate_limit_global_burst: int = 4

    # KIS 실호출 초당 빈도 계측 — 트레일링 1초 호출수가 이 값 이상이면 WARNING.
    # 전역 리미터가 한 윈도우 최대 burst + rate = 13/sec 로 묶으므로, 그보다 높은
