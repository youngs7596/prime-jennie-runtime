...
daily_quant_scores + daily_prices 를 fake conn 으로 주입, IC 산출 + 캐시 확인.
"""
...
def _price_series(code: str, start: date, opens: list[int]) -> list[dict]:
    return [
...
async def test_weekly_factor_analysis_computes_ic_and_caches():
    ...
    for i in range(12):
        code = f"{i:06d}"
        # momentum_score 0..100 비례, fwd_return 도 동일 방향
        m = 5 + i * 8  # 5..93
        scores.append(_score(code, score_date, momentum=m, news=m, supply=m))
        # 5일 후 종가 = 1000 * (1 + i*0.01) → 0%, 1%, 2%, ..., 11%
    ...
    # momentum 과 fwd_return 이 단조 증가 → spearman = 1.0
    ...
    # combined: news AND supply >= 70 → 동일하게 3개
